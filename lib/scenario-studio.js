...
function toCityCell(feature) {
  const props = feature.properties || {};
  const centroid = polygonCentroid(feature.geometry);
  const traffic2036 = trendMetric(props, "traffic");
  const jobs2036 = trendMetric(props, "jobs");
  const electricity2036 = trendMetric(props, "electricity");
  const buildings2036 = trendMetric(props, "buildings");
  const services2036 = trendMetric(props, "services");
  ...
  const populationPressure = clamp(buildings2036 * 0.72 + Number(props.development_pressure ?? 0) * 0.28);
  const environmentalExposure = clamp((1 - greenScore) * 0.48 + traffic2036 * 0.24 + electricity2036 * 0.18 + roadPressure * 0.1);
...
function cityCells(rootDir) {
  return loadCityData(rootDir).grid2026.features.map(toCityCell);
}
...
function validatePlacement(payload = {}, rootDir = process.cwd()) {
  ...
  const location = payload.location || locationFromGeometry(payload.geometry);
  ...
  const cityBbox = geometryBbox(data.grid2026);
...
function createBuildingIntervention(payload = {}, rootDir = process.cwd()) {
  ...
  const location = payload.location || locationFromGeometry(payload.geometry);
  const geometry = payload.geometry || buildSquareFootprint(location, config.footprintSqm);
  const validation = payload.validation || validatePlacement({ location, geometry, config }, rootDir);
  ...
  return {
    ...
    config,
    delivery: {
      startYear: Number(payload.delivery?.startYear || payload.startYear || 2028),
      completionYear: Number(payload.delivery?.completionYear || payload.completionYear || 2032)
    },
    validation: {
...
function runMultipleSimulations(input = {}, rootDir = process.cwd()) {
  const baseBuilding = input.building?.interventionType === "building" ? input.building : createBuildingIntervention(input.building || input, rootDir);
  const variants = sanitizeScenarioVariants(input.branches || input.variants || generateFallbackVariants({ building: baseBuilding }, rootDir), baseBuilding, rootDir);
  const cells = cityCells(rootDir);
  const contextLocation = input.location || baseBuilding.location || firstInterventionLocation(variants);
  const contextCells = getCellsWithin(cells, contextLocation, 950);
  const contextIds = new Set(contextCells.map((item) => item.cell.id));
  const baselineMetrics = summarizeMetrics(cells, contextIds);
  const baselineBranch = {
    name: "2036 Baseline",
    branchName: "2036 Baseline",
    objective: "baseline",
    description: "Current 2016-2026 trajectory projected to the 2036 scenario target.",
    interventions: [],
    assumptions: ["No new building intervention is applied."],
    metrics: baselineMetrics,
    diffFromBaseline: zeroDiff(),
    affectedCells: cellsToFeatureCollection(contextCells.map((item) => item.cell), zeroDiff()),
    agentNotes: [],
    score: 0
  };

  const simulated = variants.map((variant) => simulateVariant(variant, cells, contextIds, baselineMetrics, baseBuilding, rootDir));
  const allBranches = [baselineBranch, ...simulated];
  const recommendedBranch = chooseRecommendedBranch(allBranches);
  for (const branch of allBranches) {
    branch.recommended = branch.name === recommendedBranch.name;
  }
  return {
    ...
    contextCellIds: [...contextIds],
    context: {
      location: contextLocation,
      radiusM: 950,
      cells: contextIds.size
    }
  };
}
...
function critiqueSimulation(input = {}) {
  ...
  const humanReviewRequired = Boolean(
    siteContext.validation?.status === "warning" ||
      diff.populationPressure > 0.04 ||
      diff.environmentalExposure > 0.012 ||
      diff.electricityDemand > 0.025 ||
      diff.fairnessScore < 0
  );
  return {
...
function reportSimulation(input = {}) {
  ...
  const summary = `The original proposal raises housing capacity by increasing the population-pressure proxy ${formatDelta(userDiff.populationPressure)}. ${recommendedName} keeps the proposal testable while balancing mobility, fairness, and environmental exposure. Planner review is ${critic.humanReviewRequired || critic.human_review_required ? "recommended" : "not automatically required"} because these are proxy estimates, not engineering guarantees.`;
  return {
    headline,
    summary,
    city_commits: [
      `+ Added ${describeBuildingFromBranch(userBranch)} near the selected Belfast site`,
      `+ Compared ${Math.max(0, branches.length - 1)} scenario branches against the 2036 baseline`,
      `${diff.mobilityStrain <= userDiff.mobilityStrain ? "+" : "!"} Tested mobility mitigation for corridor strain`,
