...
function normalizeScenarioIntervention(rawIntervention = {}, baseBuilding, rootDir = process.cwd()) {
  ...
  if (type === "transformer") {
    ...
    return {
      ...
      type: "transformer",
      location,
      radiusM: Number(rawIntervention.radiusM || rawIntervention.radius_m || 650),
      rationale: rawIntervention.rationale || "User-staged transformer is included as a deterministic energy intervention."
    };
  }
  return null;
}
...
function sanitizeScenarioVariants(variants, baseBuilding, rootDir = process.cwd(), options = {}) {
  ...
  const allowedTypes = new Set(["building", "mobility_corridor", "green_corridor", "opportunity_hub", "road", "road_corridor", "transformer", "infrastructure"]);
  const sanitized = [];
...
function applyTransformerPlanner(values, context, intervention, year, distanceM) {
  const radiusM = Number(intervention.radiusM || intervention.radius_m || 650);
  const ramp = operationRamp(year, START_YEAR, HORIZON_YEAR);
  const w = plannerWeight(distanceM, radiusM) * ramp;
  if (!w) return;
  const deprivation = clamp(Number(context.cell?.deprivationWeight || 0));
  addMetricDelta(values, "electricity", -0.16 * w);
  addMetricDelta(values, "services", 0.03 * w);
  addMetricDelta(values, "economy", 0.02 * w);
  addMetricDelta(values, "jobs", 0.01 * w);
  addMetricDelta(values, "environmentAir", 0.006 * w);
  addMetricDelta(values, "fairness", 0.025 * Math.max(0.25, deprivation) * w);
  addMetricDelta(values, "fiscalBalance", -0.014 * w);
  addMetricDelta(values, "planningViability", 0.04 * w);
}
  ...
  for (const intervention of variant.interventions || []) {
    ...
    for (const item of contextRows) {
      ...
      if (type === "building") {
        ...
        applyTransformerPlanner(values, item, intervention, year, distanceM);
      }
...
function branchAgentNotes(variant, building, siteContext) {
  ...
  if ((variant.interventions || []).some((item) => {
    ...
    notes.push({ agent: "Energy Agent", risk: "medium", reason: "User-staged transformers reduce electricity pressure while adding small infrastructure cost and planning effects." });
  }
  return notes;
}
...
function buildingStatsFromVariant(variant, baseBuilding) {
  const buildingInterventions = (variant.interventions || []).filter((item) => interventionType(item) === "building");
  ...
  return sources.map((item) => {
    ...
    return deriveBuildingStats(config);
  });
}
...
function sumBuildingStats(stats = []) {
  return stats.reduce((sum, item) => {
    ...
    sum.units += Number(item.units || 0);
    sum.estimatedResidents += Number(item.estimatedResidents || 0);
    sum.estimatedJobs += Number(item.estimatedJobs || 0);
    sum.estimatedElectricityDemand += Number(item.estimatedElectricityDemand || 0);
    sum.grossFloorAreaSqm += grossFloorAreaSqm;
    sum.communityAreaSqm += grossFloorAreaSqm * Number(type.communityShare || 0);
    sum.commercialAreaSqm += grossFloorAreaSqm * Number(type.commercialShare || 0);
    sum.residentialAreaSqm += grossFloorAreaSqm * Number(type.residentialShare || 0);
...
function roadConcreteStats(interventions = [], year, baselineTraffic = 0) {
  ...
  for (const intervention of interventions) {
    ...
    count += 1;
    lengthM += Number(intervention.lengthM || intervention.length_m || pathLengthMeters(intervention.path || []));
  }
  const lengthKm = lengthM / 1000;
  const capacityDailyTrips = lengthKm * 1800 * ramp;
  const inducedDailyTrips = capacityDailyTrips * clamp(0.24 + Number(baselineTraffic || 0) * 0.22, 0.2, 0.42);
  const dailyTripRelief = capacityDailyTrips * 0.58;
  return {
    count,
    ...
    inducedDailyTrips,
    dailyTripRelief,
    accessJobs: lengthKm * 28 * ramp,
    serviceCapacityEquivalent: lengthKm * 38 * ramp,
    annualLightingMwh: lengthKm * 5.5 * ramp
  };
}

function transformerConcreteStats(interventions = [], year) {
  const ramp = operationRamp(year, START_YEAR, HORIZON_YEAR);
  const count = (interventions || []).filter((item) => interventionType(item) === "transformer").length;
  return {
    count,
    transformerReliefKw: count * 900 * ramp,
    serviceCapacityEquivalent: count * 35 * ramp,
    operationsJobs: count * 1.5 * ramp
  };
}

function concreteImpactsForVariant(variant, baseBuilding, siteContext, year, diffFromBaseline, baselineMetrics) {
  const ramp = operationRamp(year, START_YEAR, HORIZON_YEAR);
  ...
  const transformerStats = transformerConcreteStats(interventions, year);
  ...
  const netDailyTrips = grossDailyTrips + roadStats.inducedDailyTrips - roadStats.dailyTripRelief + forecastTrafficSignal * 0.25;
  const peakHourVehicleChange = netDailyTrips * 0.11;
  const delayMinutesPerPeakHourChange = Number(diffFromBaseline.traffic || 0) * 160;
  ...
  const accessibilitySupportedJobs = roadStats.accessJobs + opportunityJobs + transformerStats.operationsJobs + forecastSupportedJobs * 0.25;
  const annualMwhChange = (buildingStats.estimatedElectricityDemand * energyFactor + roadStats.annualLightingMwh) * ramp;
  const peakKwChange = annualMwhChange > 0 ? annualMwhChange * 1000 / (8760 * 0.42) : 0;
  ...
  return {
    year,
    modelBasis: "2016-2025 trained forecast artifact plus deterministic planners",
    ...
    traffic: {
      ...
      congestionIndexDelta: round(diffFromBaseline.traffic || 0, 3),
      delayMinutesPerPeakHourChange: roundConcrete(delayMinutesPerPeakHourChange, 1),
      roadLengthM: roundConcrete(roadStats.lengthM, 1),
      method: "trip generation + road capacity relief + induced demand + forecast traffic delta"
    },
    jobs: {
      ...
      method: "building employment density + road/accessibility uplift + trained jobs forecast delta"
    },
    electricity: {
      ...
      localLoadIndexDelta: round(diffFromBaseline.electricity || 0, 3),
      method: "floor-area electricity demand + road lighting load + transformer relief + forecast load delta"
    },
    services: {
    ...
    evidence: [
      ...
      "lib/scenario-studio.js deterministic planners"
    ],
    warning: "Concrete values are calibrated planning estimates, not engineering-grade traffic assignment, grid-capacity approval, or statutory service modelling."
  };
}
...
function simulateForecastVariant(variant, contextRows, baselineTimeline, baseBuilding, siteContext, rootDir) {
  ...
  for (const year of FORECAST_YEARS) {
    ...
    const concreteImpacts = concreteImpactsForVariant(variant, baseBuilding, siteContext, year, diffFromBaseline, baselineMetrics);
...
function evidenceForScenarioBranch(variant, building) {
  ...
  for (const intervention of variant.interventions || []) {
    ...
    if (type === "transformer" || type === "infrastructure") evidence.push("Energy planner: transformer demand relief, service resilience, infrastructure cost and planning effects");
  }
